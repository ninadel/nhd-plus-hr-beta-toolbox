...
class TributaryFinder(object):
    ...
    def execute(self, parameters, messages):
            ...
            for field in gfcode_fields:
                ...
                if len(gnisid_list) > 0:
                    arcpy.SelectLayerByAttribute_management(
                        in_layer_or_view=fc,
                        selection_type="NEW_SELECTION",
                        where_clause="GNIS_ID IN {0} AND LevelPathI IN {1}".format(
                            get_list_string(gnisid_list, True)[0],
        ...
        result_startfeatures_dissolved_location = get_location(result_gdb_location,
                                                               result_startfeatures_dissolved_fcname)
        result_tributaries_segments_location = get_location(result_dataset_location, result_tributaries_segments_fcname)
        result_tributaries_dissolved_location = get_location(result_dataset_location,
                                                             result_tributaries_dissolved_fcname)

        ### Processing Phase 1: Pre-tests
        print('Processing Phase 1: Pre-tests')
        # do usable start features exist
        start_nhdplusids = find_start_features(original_nhdflowline_location, parameter_start_feature,
                                               result_startfeatures_segments_location)
        # if there are usable start features
        if len(start_nhdplusids) > 0:
            # find tributaries that match parameters, if any
            vaa_lists = get_vaa_lists(original_nhdflowline_name, original_vaatable_location, start_nhdplusids,
                                      parameter_all_levels,
                                      parameter_max_level)
